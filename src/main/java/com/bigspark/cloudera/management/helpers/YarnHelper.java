...
public class YarnHelper {
  ...
  /**
   * Method to manage Livy Spark Applications Currently Deletes Sessions idle for > 120 minutes
   *
   * @param args
   * @throws Exception
   */
  private static void manageLivySparkApps(String[] args) throws Exception {
    ...
    try (YarnClient yarn = getYarnClient()) {
      yarn.start();
      for (ApplicationReport report : yarn
  ...
  /**
   * Method to get a Yarn Client instance Note it is expected that the required configs are on the
   * class path
   *
   * @return Yarn Client
   */
  ...
  /**
   * Method to determine whether to kill an application or not An application will be killed that
   * meets the following critera 1. Most recent job completed > 120 mins ago 2. Has no jobs, and was
   * started > 120 mins ago
   *
  ...
  private static Boolean isKillIdleApplication_NEW(ApplicationReport report)
    ...
    SparkJobDto[] jobs = getJobs(
        String.format("%sapi/v1/applications/%s/jobs", url, applicationId));
    ...
    if (jobs != null && jobs.length > 0) {
      ...
      if (!latestJob.getStatus().equals("RUNNING")) {
        ...
        LocalDateTime jobCompletionTime = LocalDateTime
            .parse(completionTime.replace("GMT", ""), formatter);
        killApplication = Duration.between(jobCompletionTime, now).toMinutes() >= minsToKill;
        if (killApplication) {
          LOGGER.info(String.format(
              "App id '%s' has a latest job completion time of '%s'.  Time now is '%s'.  This is > %d minutes, so will kill",
              applicationId, jobCompletionTime, now, minsToKill));
        } else {
          LOGGER.info(String.format(
              "App id '%s' has a latest job completion time of '%s'.  Time now is '%s'.  This is <= %d minutes, so will not kill",
              applicationId, jobCompletionTime, now, minsToKill));
        }
      } else {
        LOGGER.info(String
            .format("Application Id %s has currently Running Jobs.  Ignoring.", applicationId));
      }
           /*
      ...
      LocalDateTime appStart = LocalDateTime
          .ofInstant(Instant.ofEpochMilli(report.getStartTime()), TimeZone.getDefault().toZoneId());
      killApplication = Duration.between(appStart, LocalDateTime.now()).toMinutes() >= minsToKill
          && report.getApplicationResourceUsageReport().getNumUsedContainers() == 1;
      if (killApplication) {
        LOGGER.info(String.format(
            "App id '%s' has no jobs.  App Start Time is '%s'. Time now is '%s'.  This is > %d minutes, so will kill",
            applicationId, appStart, now, minsToKill));
      } else {
        LOGGER.info(String.format(
            "App id '%s' has no jobs.  App Start Time is '%s'. Time now is '%s'.  This is <= %d minutes, so will not kill",
            applicationId, appStart, now, minsToKill));
      }
    }
