...
public class FileSystemHelper {
  ...
  public static FileSystem getConnection() throws IOException {
    if (fs == null) {
      fs = FileSystem
          .newInstance(SparkHelper.getSparkSession().sparkContext().hadoopConfiguration());
    }
    return fs;
  }
  ...
  public static void writeFileContent(String targetPath, String fileName, String payload,
      Boolean deleteFile) throws IllegalArgumentException, IOException {
  ...
  private static FSDataOutputStream getFile(FileSystem fs, String fullPath)
  ...
  public static String getCreateTrashBaseLocation(String jobType, String dbName, String tableName)
      throws IOException {
    FileSystem fileSystem = FileSystemHelper.getConnection();
    String userHomeArea = FileSystemHelper.getUserHomeArea();
    long seconds = System.currentTimeMillis() / 1000l;
    String location = String
        .format("%s/.ClusterManagementTrash/%s/%s/%s/%d", userHomeArea, jobType, dbName, tableName,
            seconds);
  ...
  public static Boolean moveDataToUserTrashLocation(String sourceLocation, String trashBaseLocation,
      Boolean isDryRun, FileSystem fileSystem) throws URISyntaxException, IOException {
    String trashTarget = trashBaseLocation + sourceLocation;
    URI trashTargetURI = new URI(trashTarget);
    String trashTargetParent =
        trashTargetURI.getPath().endsWith("/") ? trashTargetURI.resolve("..").toString()
            : trashTargetURI.resolve(".").toString();
