...
import com.bigspark.cloudera.management.jobs.offload.OffloadController;
...
public class TstDataSetup {
    ...
    private void createTestTables() throws IOException {
        ...
        if (!spark.catalog().tableExists(testingDatabase, "test_table_sh")) {
            ...
            csv.write().partitionBy("edi_business_day").mode("overwrite").saveAsTable(testingDatabase+".test_table_sh");
            spark.sql("SHOW TABLES").show();
        }
        if (!spark.catalog().tableExists(testingDatabase, "test_table_eas")) {
            System.out.println("Creating EAS test table...");
            csv.write().partitionBy("edi_business_day", "src_sys_id", "src_sys_inst_id").mode("overwrite").saveAsTable(testingDatabase+".test_table_eas");
            spark.sql("SHOW TABLES").show();
        }
    }
